...
module.exports = class BaseApi extends Base {
  ...
  endpoint (path, data = {}) {
    const pathParamReplace = match => {
      const key = match.replace(/[\{\}]/g, '')
      const value = data[key]
      delete data[key] // so it is not repeated in the querystring
      return value
    }
    const pathWithParams = path.replace(/\{[a-z0-9_-]+\}/g, pathParamReplace)
    return `${this.baseUrl}/${pathWithParams}`
  }
  ...
  standardOptions () {
    ...
    return {
      headers: headers,
      mimetypes: self.config.mimeTypes || {
  ...
  defaultHeaders () {
    const self = this
    return {
      'Content-Type': self.config.contentType || 'application/json'
    }
  }
  ...
  callApi (method, data, forceParameters) {
    ...
    const self = this
    const options = self.standardOptions()
    ...
    const endpoint = self.endpoint(self.path, data)
  ...
  async setCache (params, data) {
    ...
    if (!this.config.db) {
      throw ('cannot cache api responses unless a db for cache is set in config.db')
    }

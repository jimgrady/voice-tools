...
class AirtableContent {
  ...
  template (key, data, userHourOfDay = null, recursiveCall = false) {

    let string = key
    // for top level calls we want to grab the full template based on key
    // for recursive calls we don't want to substitute unless we have curly braces like {key}
    if ((!recursiveCall) && (typeof data[key] !== 'undefined')) {
      string = data[key]
    }
    ...
    const replaced = string.replace(/\{[^\}]*\}/g, (match) => {
      let out
      let key = matchToKey(match)
      if (key.charAt(key.length - 1) === '*' && !key.includes('&&') && !key.includes('&!')) { // ends in wildcard - support varying across all matches
      ...
      if (keyParts.length >= 3) { // e.g. hoa:true:Yes, there is an HOA
        ...
        if (value === '>0') {
          ...
          const val = Number(value.split('>')[1])
          if (parseInt(data[key]) > val) {
            matched = true
          }
        } else if (value.includes('!=')) {
          const str = value.split('!=')[1]
          if (data[key] !== str) matched = true
        } else if (value !== 'none' && data[key] === null) {
          return ''
        } else if (value === 'any' && data[key] && data[key] !== 'none') {
          matched = true
        } else if (data[key] && data[key].toString() == value) {
          matched = true
        } else if (data[key] === Number(value)) {
          matched = true
        } else if (value === 'no' && data[key] === false || value === 'yes' && data[key] === true) {
          matched = true
        } else {
          matched = false
        }
        // do following line if fields val isn't null (or ''?)
        // if it is, and val not none, then return ''

...
class AirtableContent {
  ...
  template (key, data, userHourOfDay = null, templateId, reservedWordConfig) {
    let string
    ...
    if (reservedWordConfig && reservedWordConfig.reservedWords.includes(key) && reservedWordConfig.fieldsWithNoReservedWords.includes(templateId.currentField)) {
      return key
    }
    else if (!matches) {
    ...
    const replaced = string.replace(/\{[^\}]*\}/g, (match) => {
      ...
      out = this.template(out, data, userHourOfDay, templateId, reservedWordConfig)
      return out
    })
    return replaced
  }

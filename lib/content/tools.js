...
module.exports = class ContentTools extends Base {
  ...
  selectKeyVariation (key, data, userHourOfDay) {
    ...
    selectedVariationKey = variations[Math.floor(Math.random() * variations.length)]
  ...
  renderTemplateTag (match, data, userHourOfDay) {
    ...
    if (self.isAutomaticallyVaryingKey(key)) {
      key = self.selectKeyVariation(selectKeyVariation(key, data, userHourOfDay))
    }
  ...
  renderCompoundTag (key, data) {
    ...
    // only the special condition values 'any' and 'none' are allowed to operate on null values,
    if (
      !(condition === 'any' || condition === 'none')
    ...
    if (condition === '>0') {
      ...
      if (self.isValidValue(baseValue) && baseValue !== 'none') {
        matched = true
      }
    } else if (condition === 'none') {
      // TODO: remove exception for "none" value
      if ((!self.isValidValue(baseValue)) || (baseValue === 'none')) {
        matched = true
      }
  ...
  autoSingularOrPlural (input, baseValue) {
    ...
    const isAre = parseInt(baseValue) > 1 ? 'are' : 'is'

...
const ContentSource = require('../../../content/source/airtable')
...
class BaseHandler extends Base {
  constructor (handlerInput, config, logInstance) {
    ...
    this.utils = utils
    this.platformUtils = this.config.platformUtils || platformUtils // make configurable for testing
    this.contentTools = new ContentTools(config)
    this.contentSource = new ContentSource(config)
    ...
    this._analyticsUserProperties = null
    if (this.config.varyResponsesByTimeOfDay) {
  ...
  template (id, tempData, keepNewLines) {
    const self = this
    let out = this.contentTools.template(id, self.templateData(tempData), self.userHourOfDay)
    if (out) {
      // newlines cause a long pause, quotes must be encoded in SSML
      if (!keepNewLines) {
        out = out.replace(/\n/g, ' ')
      }
      // &, #, and " characters can break SSML
      // any changes here have to be safe to run repeatedly since templates can be nested
      out = out.replace(/&(?![a-z]{3,4};)/g, ' and ') // an ampersand that isn't the start of a character entity
      out = out.replace(/(")(?![^<]*>|[^<>]*<\/)/g, '&quot;')
      out = out.replace(/#/g, ' number ')
    }
    if (!out) {
      const debugMessage = `could not load template content - check that all replacments are available for ${id} : ${this.uiTemplates[id] === undefined ? '' : this.uiTemplates[id]}`
  ...
  async after () {
    ...
    if (this.analytics && (!this.customAnalytics)) {
      ...
      await this.analytics.trackEvent(this, this.analyticsProperties(), this.analyticsUserProperties())
    }
  }

  templateRespond (id, tempData = {}, endSession = true, repromptId = null) {
    const out = this.templateResponse(id, tempData, endSession, repromptId)
    if (out === '') {
  ...
  conditionalResponse (id, filterState, mergeState = null, preResponseCallback = null, endSession = false, postResponseCallback = null) {
    const self = this

    // repeat intent support
    const lastResponseId = this.getState('last-response-id')
    const intentCategory = this.getState('intent-category')
    if (id === 'repeat-intent') {
      const out = this.templateResponse(lastResponseId, {}, endSession, null, true)
      if (intentCategory === 'search' && postResponseCallback) {
        // this callback is needed for search but breaks facts
        postResponseCallback(out)
      }
      return out.getResponse()
    }

    if (filterState !== 'default' && !self.stateMatches(filterState, id)) {
      self.log('DEBUG', `no match for ${id}`)
      return false
    }
    self.log('DEBUG', `match for ${id}`)
    if (preResponseCallback) {
      preResponseCallback(self)
    }
    if (mergeState && (Object.keys(mergeState).length > 0)) {
      this.setState(mergeState)
    }
    const out = this.templateResponse(id, {}, endSession, null)
    if (out === '') {
      throw ({ message: 'empty template in templateRespond - check logs for missing replacements' })
    }
    if (postResponseCallback) { // should really be called postResponseGenerationCallback
      postResponseCallback(out)
    }
    this.setLastResponseId(id)
  ...
  templateResponse (id, tempData, endSession = true, repromptId = null, repeatPrompt = false) {
    const self = this

    // repeat intent support
    const lastNextSteps = this.getState('last-next-steps')
    if (repeatPrompt === true) {
      self.standardNextSteps = lastNextSteps
    } else this.handleFollowup(id, endSession)
    ...
    this.setSessionState({
      'last-next-steps': self.standardNextSteps
    })
  ...
  /*
     ...
     */
  handleFollowup (templateId, endSession) {
    const self = this
    if (endSession) {
      ...
      self.standardNextSteps = self.generateStandardNextSteps(templateId)
      self.standardReprompt = self.generateStandardReprompt(templateId)
      self.standardGoodbye = ''
    }
  }

  generateStandardReprompt (templateId) {
    return this.standardReprompt
  }

  generateStandardNextSteps (templateId) {
  ...
  slots () {
    const request = this.request(); let slots
  ...
  delegateDialog (response = null, slots = {}) {
    ...
    for (const key in intent.slots) {
      if (slots[key]) {
        Object.assign(intent.slots[key], slots[key])
      }
    }
  ...
  async respond (speechText, shouldEndSession = false, repromptText = null) {
    ...
    const out = this.response(speechText, shouldEndSession, repromptText)
    // this._analyticsProperties._message = speechText;
    this.log('DEBUG', 'RESPONSE_TEXT', speechText)
    return out.getResponse()
  }

  response (speechText, endSession = false, repromptText = null) {
    const out = this.handlerInput.responseBuilder
      .speak(speechText)
  ...
  saveAttributes () {
    if (!this.userId()) {
      this.log('ERROR', 'no user id, could not save attributes')
      return
    }
  ...
  static canHandle (handlerInput) {
    ...
    if (requestedIntent) {
      nameCheck = false
      const intentName = self.intentName()
      if (!intentName) {
        nameCheck = false
    ...
    const can = typeMatches && nameCheck && self.canHandleFilter(handlerInput)
    return can
  }
  ...
  static canHandleFilter (handlerInput) {
    ...
    const rule = rules[self.requestedIntentName(handlerInput)] || rules.ANY_INTENT
    if (!rule) {
      return true
    }
  ...
  slotListValue (slotName) {
    const slot = this.slots()[slotName]?.slotValue
    const out = []
    if ((!slot) || (slot.type !== 'List')) return null
    slot.values.forEach(el => {
      if (el.resolutions.resolutionsPerAuthority[0].status.code === 'ER_SUCCESS_MATCH') {
        out.push(el.resolutions.resolutionsPerAuthority[0].values[0].value.name)
      }
    })
    return out.join(',')
  }
  ...
  templateData (overrides) {
    ...
    if (this.attributes !== undefined) {
      ...
      const allState = self.getState()
      if (typeof allState === 'object') {
        const factTemplates = allState['fact-templates']
        delete allState['fact-templates']
        if (typeof factTemplates === 'object') {
          for (const key in factTemplates) {
            data[`${key}-response`] = factTemplates[key].template
          }
        }
        Object.assign(data, allState)
      }
    ...
    if (self.config.allStateInTemplateScope !== false) {
  ...
  async loadDynamicConfig () {
    ...
    this.userHourOfDay = null
    if (this.config.varyResponsesByTimeOfDay) {
      const timezone = await this.platformUtils.getTimezone(this.handlerInput)
      if (timezone) {
        this.userHourOfDay = this.momentTimezone().tz(timezone).hour()
      }
    }
  }
  ...
  before () {
    return this.loadDynamicConfig()
      ...
      .then(() => {
        if (this.analytics && (!this.customAnalytics)) {
          this.analytics.trackEvent(this, this.analyticsProperties(), this.analyticsUserProperties())
        }
        return Promise.resolve()
      })
      .then(() => {
      ...
      .catch((error) => {
        return this.catchRespond(
              `Sorry, I'm still setting a few things up. I should be ready in a couple of seconds. To try again, please say 'open ${this.invocationName()}.'`,
              error
        )
      })
  }
  ...
  analyticsUserProperties () {
    return this._analyticsUserProperties
  }
  ...
  loadState () {
    ...
    const lastIntent = this.getState('current-intent-name')
    this.setSessionState({
      'last-intent-name': lastIntent === 'repeat-intent' ? this.getState('last-intent-name') : lastIntent,
      'current-intent-name': this.requestedIntentName()
    })
  ...
  loadTemplates (force) {
    ...
    this.config.uiTemplates['invocation-name'] = this.invocationName()
    ...
    return this.loadCachedUiTemplates()
      .then(result => {
        if (result) {
          ...
          return this.contentSource.loadUiTemplates(this, this.isLaunchRequest())
            .then(() => {
      ...
      .then(_ => {
        this.uiTemplates['invocation-name'] = this.invocationName()
  ...
  stateKey (forceMode = null) {
    ...
    if (!deviceId) {
      ...
      self.log('WARN', 'STATE_MANAGEMENT', 'cannot set device-level state in a non-device request. Defaulting to user state.')
      return 'state'
    }
  ...
  setState (state, persist = true, saveNow = null, returnSync = false, scope = null) {
    ...
    if (this.sessionId() && (persist !== 'only')) { // add to session for in-session requests - todo - maybe separate session
      const sessionAttributes = this.attributesManager.getSessionAttributes()
  ...
  userId () {
    if (!this.userInput()) {
      return null // e.g. CanFulfillIntentRequest without skill installed has no user id
    }
  ...
  loadAccessToken () {
    ...
    try {
      // noinspection JSUnresolvedVariable
      const accessToken = this.userInput().accessToken
      if (accessToken) {
  ...
  lastResponseId () {
    return this.constructor.lastResponseId(this.handlerInput)
  }
  ...
  invocationName() {
    const self = this
    const invocationName = self.config.invocationName
    if (typeof invocationName !== 'object') {
      return invocationName
    }
    if (invocationName[this.skillId()]) {
      return invocationName[this.skillId()]
    }
    return invocationName.default
  }
}

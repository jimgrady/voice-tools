...
class BaseHandler extends Base {
    ...
    conditionalResponse(id, filterState, mergeState = {}, callback = null) {
        const self = this;
    ...
    uiTemplateKey() {
        const self = this;
        const version = self.config.uiTemplateVersion ? self.config.uiTemplateVersion : 1;
    ...
    loadCachedUiTemplates(skip = false) {
        const self = this;
        if (self.isLaunchRequest() && self.config.isDev) {
            // reload every launch in dev
            return Promise.resolve(null);
        }
        if (self.config.cachedUiTemplates) {

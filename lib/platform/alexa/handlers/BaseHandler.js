...
class BaseHandler {
    ...
    static canHandle(handlerInput) {
        ...
        this.log(`${this.name} ${can ? "CAN " : 'CANNOT'} HANDLE ${request.type} : ${request.intent ? request.intent.name : '[no name]'}`);
        return can;
    }
    static intentName() {
        return null;
    ...
    templateData(tempData) {
        ...
        const slotValues = this.allSlotValues();
        Object.keys(slotValues).forEach(key => {
            if (data[key]) {
                throw(`template id conflict: slot key '${key}' already used in scope`);
            }
            data[key] = slotValues[key];
        });
        if (tempData) {

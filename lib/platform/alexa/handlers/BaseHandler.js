...
class BaseHandler extends Base {
    ...
    catchRespond(speech, error, endSession) {
    ...
    loadTemplates(force) {
        ...
        if ((!this.attributes.uiTemplates) || this.isLaunchRequest()) {
    ...
    stateMatches(state, name) {
        ...
        for (let key in state) {
            let val = state[key];
            if (Array.isArray(val) && (val.indexOf(this.attributes.state[key]) === -1)) {
                this.log('DEBUG', `failed on ${key}: current state ${this.attributes.state[key]} not in array ${val.join(', ')}`);
                return false;
            }
            if (this.attributes.state[key] !== val) {

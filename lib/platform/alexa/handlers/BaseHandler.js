...
class BaseHandler extends Base {
    ...
    stateMatches(state, name) {
        ...
        for (let key in state) {
            if (Array.isArray(state[key]) && (state[key].indexOf(this.attributes.state[key]) === -1)) {
                this.log('DEBUG', `failed on ${key}: current state ${this.attributes.state[key]} not in array ${state[key].join(', ')}`);
                return false;
            }
            if (this.attributes.state[key] !== state[key]) {
                this.log('DEBUG', `failed on ${key}: current state ${this.attributes.state[key]} !== ${state[key]}`);
                return false;
            }
        }

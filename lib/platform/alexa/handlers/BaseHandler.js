...
class BaseHandler extends Base {
    ...
    conditionalResponse(id, filterState, mergeState = {}, callback = null, endSession = false, responseCallback = null) {
        const self = this;
        if (filterState !== 'default' && !self.stateMatches(filterState, id)) {
            return false;
        }
        const defaultMergeState = {"last-response-id": id};

        let updateState = Object.assign(defaultMergeState, mergeState);
        if (callback) {
            callback(self);
        }
        this.setState(updateState, true, false, true);
    ...
    static lastResponseId(handlerInput) {
        ...
        && handlerInput.requestEnvelope.session.attributes.state) {
            return handlerInput.requestEnvelope.session.attributes.state['last-response-id'] || null;
        } else {
            return null;
        }
    }

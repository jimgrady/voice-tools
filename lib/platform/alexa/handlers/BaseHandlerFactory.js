...
class BaseHandlerFactory {
  ...
  canHandle (handlerInput) {
    ...
    logInstance.log(can ? 'INFO' : 'DEBUG', 'ROUTING', `${this.handlerClass.name} ${can ? 'CAN' : 'CANNOT'} HANDLE ${request.type} : ${request.intent ? request.intent.name : '[no name]'} : ${request.requestId}`)
    return can
  }
  ...
  handle (handlerInput) {
    ...
    try {
      ...
      logInstance.log('ERROR', `Handler Factory Handle Error: ${e}`)
    }
  }
}
